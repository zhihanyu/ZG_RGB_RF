...
uint8_t dhcp_option_lease_time[4] = {0x00, 0x00, 0x1c, 0x20}; //1 day
struct ip_addr dhcps_addr_pool_start;
struct ip_addr dhcps_addr_pool_end;
...
static mutex_t dhcps_ip_table_semaphore;

static struct netif * dhcps_netif = NULL;

extern void at_wlan_add_ip(struct ip_addr ip,u8 *bssid);
/**
...
static void mark_ip_in_table(uint8_t d)
{
  	...
  	printf("\r\nmark ip %d\r\n",d);
#endif	
	wl_lock_mutex(&dhcps_ip_table_semaphore);
	if (0 < d && d <= 32) {
	...
	wl_unlock_mutex(&dhcps_ip_table_semaphore);
...
static uint8_t check_client_request_ip(struct ip_addr *client_req_ip, uint8_t *hwaddr)
{
    ...
    start = ip4_addr4(&dhcps_addr_pool_start);
    end   = ip4_addr4(&dhcps_addr_pool_end);
...
/**
  ...
  * @param: None 
  * @retval the usable index which represent the ip_addr(ip) of allocated ip addr.
  */
#if (!IS_USE_FIXED_IP)
...
{       
	...
	for (range_count = 0; range_count < 8; range_count++) {
	...
	uint8_t option_code, uint8_t option_length, void *copy_info)
{
	...
	default:
        next_option_start_address = option_data_base_address;
        break;
...
static void add_offer_options(uint8_t *option_start_address)
{
	...
	temp_option_addr = fill_one_option_content(temp_option_addr,
		DHCP_OPTION_CODE_PERFORM_ROUTER_DISCOVERY, 0,
								NULL);
...
static void dhcps_send_offer(struct pbuf *packet_buffer)
{
	...
	printf("\n\r[%d]DHCP assign ip = %d.%d.%d.%d\n", wl_get_systemtick(), ip4_addr1(&dhcps_network_id),ip4_addr2(&dhcps_network_id),ip4_addr3(&dhcps_network_id),temp_ip);
...
void dhcps_get_addr_pool(struct ip_addr * addr_pool_start, struct ip_addr *addr_pool_end)
{
    memcpy(addr_pool_start, &dhcps_addr_pool_start, sizeof(struct ip_addr));
    memcpy(addr_pool_end, &dhcps_addr_pool_end, sizeof(struct ip_addr));
}
...
int dhcps_set_iprange(struct ip_addr *first_ipaddr, struct ip_addr *last_ipaddr)
{
    u32 addr1, addr2;
    ...
    addr1 =  htonl(first_ipaddr->addr);
    addr2 =  htonl(last_ipaddr->addr);
    if(addr1 > addr2) {
        return -1;
    }
	dhcps_owned_first_ip.addr = first_ipaddr->addr;
	dhcps_owned_last_ip.addr  = last_ipaddr->addr;    
    
    return 0;
}
 
int dhcps_get_iprange(struct ip_addr *first_ipaddr, struct ip_addr *last_ipaddr)
{
    if(!first_ipaddr ||
       !last_ipaddr  ) {
        return -1;
	...
	first_ipaddr->addr = dhcps_owned_first_ip.addr;
	last_ipaddr->addr  = dhcps_owned_last_ip.addr;     
    
    return 0;
}

void dhcps_get_lease_time(uint32_t *out_lease_sec)
{
    uint8_t idx = 0;
    ...
    for(idx = 0; idx < 4; idx++)
    {
        lease_sec += ((uint32_t)dhcp_option_lease_time[idx]) << ((3 - idx) * 8);
    }
    
    *out_lease_sec = lease_sec;
}

void dhcps_set_lease_time(const uint32_t lease_sec)
{
    dhcp_option_lease_time[0] = (lease_sec >> 24) & 0xFF;
    dhcp_option_lease_time[1] = (lease_sec >> 16) & 0xFF;
    dhcp_option_lease_time[2] = (lease_sec >> 8) & 0xFF;
    dhcp_option_lease_time[3] = (lease_sec >> 0) & 0xFF;
}
...
//	dump_client_table();
    ...
    //default setting
    if(!dhcps_owned_first_ip.addr || !dhcps_owned_last_ip.addr) {
  ...
  dhcps_pcb->so_options|=SOF_BROADCAST;
	...
	if (dhcps_ip_table_semaphore != NULL) {	
		wl_free_mutex(&dhcps_ip_table_semaphore);
		dhcps_ip_table_semaphore = NULL;
	}
	wl_init_mutex(&dhcps_ip_table_semaphore);
	memset(&ip_table, 0, sizeof(struct table));
	mark_ip_in_table((uint8_t)ip4_addr4(&dhcps_local_address));
	...
	{		
        dhcps_addr_pool_start.addr = ip4_addr4(&dhcps_owned_first_ip);
        dhcps_addr_pool_end.addr   = ip4_addr4(&dhcps_owned_last_ip);
        //sync first and last
		memcpy(&dhcps_pool_start,&dhcps_owned_first_ip,sizeof(struct ip_addr));

		memcpy(&dhcps_pool_end, &dhcps_owned_last_ip,sizeof(struct ip_addr));
