...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if( ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            correct &= ( padding_idx < ssl->in_content_len +
                                       ssl->transform_in->maclen );
...
//    {
//        mbed_tls_reset_in_buf_len(ssl, nb_want + 100);
//    }
...
//                len = 1024;
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= fetch input" ) );
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "final in_left: %d, nb_want: %d", ssl->in_left, nb_want ) );
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->in_msglen > ssl->in_content_len
                         - (size_t)( ssl->in_msg - ssl->in_buf ) )
    {
        mbed_tls_reset_in_buf_len(ssl, ssl->in_msglen + 100);
    }
    ...
    {
        if( ssl->in_msglen < 1 ||
            ssl->in_msglen > ssl->in_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
/* 简化业务场景和编码复杂度，目前只支持增长 */
void mbed_tls_reset_in_buf_len(mbedtls_ssl_context *ssl, unsigned int new_len)
{
    MBEDTLS_SSL_DEBUG_MSG( 1, ( " in buf modify to %u", new_len) );

    unsigned int new_in_content_len = new_len;
    unsigned int new_in_buf_len = MBEDTLS_SSL_HEADER_LEN \
                                + new_in_content_len   \
                                + MBEDTLS_SSL_COMPRESSION_ADD            \
                                + MBEDTLS_MAX_IV_LENGTH                  \
                                + MBEDTLS_SSL_MAC_ADD                    \
                                + MBEDTLS_SSL_PADDING_ADD;
    if(ssl->in_buf_len >= new_in_buf_len)
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "curr in buf len > new buf %u > %u", ssl->in_buf_len, new_in_buf_len) );
        return;
    }

    unsigned char *new_in_buf = mbedtls_calloc( 1, new_in_buf_len );
    if(new_in_buf == NULL)
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "malloc fail %u", new_in_buf_len) );
        return;
    }
    if(ssl->in_buf != NULL)
    {
        memcpy(new_in_buf, ssl->in_buf, ssl->in_buf_len);
        mbedtls_free(ssl->in_buf);

        int ctr_offset = (unsigned long)ssl->in_ctr - (unsigned long)ssl->in_buf;
        int hdr_offset = (unsigned long)ssl->in_hdr - (unsigned long)ssl->in_buf;
        int len_offset = (unsigned long)ssl->in_len - (unsigned long)ssl->in_buf;
        int iv_offset = (unsigned long)ssl->in_iv - (unsigned long)ssl->in_buf;
        int msg_offset = (unsigned long)ssl->in_msg - (unsigned long)ssl->in_buf;
//        int offt_offset = (unsigned long)ssl->in_offt - (unsigned long)ssl->in_buf;

        ssl->in_buf = new_in_buf;
        ssl->in_buf_len = new_in_buf_len;
        ssl->in_content_len = new_in_content_len;
...
/* tls目前支持fragment，因此发送不会出现过长现象，该接口目前只用于初始化 */
void mbed_tls_reset_out_buf_len(mbedtls_ssl_context *ssl, unsigned int new_len)
{
    MBEDTLS_SSL_DEBUG_MSG( 1, ( " out buf modify to %u", new_len) );

    unsigned int new_out_content_len = new_len;
    unsigned int new_out_buf_len = MBEDTLS_SSL_HEADER_LEN \
                                + new_out_content_len   \
                                + MBEDTLS_SSL_COMPRESSION_ADD            \
                                + MBEDTLS_MAX_IV_LENGTH                  \
                                + MBEDTLS_SSL_MAC_ADD                    \
                                + MBEDTLS_SSL_PADDING_ADD;
    if(ssl->out_buf_len >= new_out_buf_len)
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "curr out buf len > new buf %u > %u", ssl->out_buf_len, new_out_buf_len) );
        return;
    }

    unsigned char *new_out_buf = mbedtls_calloc( 1, new_out_buf_len );
    if(new_out_buf == NULL)
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "malloc fail %u", new_out_buf_len) );
        return;
    }
    if(ssl->out_buf != NULL)
    {
        memcpy(new_out_buf, ssl->out_buf, ssl->out_buf_len);
        mbedtls_free(ssl->out_buf);
    }

    ssl->out_buf = new_out_buf;
    ssl->out_buf_len = new_out_buf_len;
    ssl->out_content_len = new_out_content_len;
                       ...
                       const mbedtls_ssl_config *conf )
{
    ...
    ssl->in_buf = NULL;
    ssl->out_buf = NULL;
    mbed_tls_reset_in_buf_len(ssl, MBEDTLS_SSL_MAX_CONTENT_LEN);
    mbed_tls_reset_out_buf_len(ssl, MBEDTLS_SSL_MAX_CONTENT_LEN);
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", MBEDTLS_SSL_MAX_CONTENT_LEN ) );
        mbedtls_free( ssl->in_buf );
        ssl->in_buf = NULL;
        return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
    }
...
static int ssl_session_reset_int( mbedtls_ssl_context *ssl, int partial )
{
    ...
    memset( ssl->out_buf, 0, ssl->out_buf_len );
    if( partial == 0 )
...
uint32_t mbedtls_ssl_get_verify_result( const mbedtls_ssl_context *ssl )
{
    if( ssl->session != NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "ssl->session != NULL" ) );
        return( ssl->session->verify_result );
    }

    if( ssl->session_negotiate != NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "ssl->session_negotiate != NULL" ) );
...
//    mbedtls_debug_print_msg( ssl, 2,"mem_check", __LINE__,"free_mem_size:%d\r\n", SysGetHeapSize());
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_buf != NULL )
    {
        mbedtls_zeroize( ssl->out_buf, ssl->out_buf_len );
        mbedtls_free( ssl->out_buf );
        ssl->out_buf_len = 0;
    }
    ...
    if( ssl->in_buf != NULL )
    {
        mbedtls_zeroize( ssl->in_buf, ssl->in_buf_len );
        mbedtls_free( ssl->in_buf );
        ssl->in_buf_len = 0;
    }
...
void mbedtls_ssl_free_session_negotiate_peer_cert( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->session_negotiate )
    {
        mbedtls_ssl_session_free_peer_cert( ssl->session_negotiate );
    }
}
