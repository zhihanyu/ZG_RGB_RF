                                      ...
                                      unsigned char *end )
{
    ...
    *p += 2;
    ...
    int fix_len = strlen("1dHRsc2NjbHltbGx3eWh5");
    if(len != fix_len + 16)
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ("server psk hint err:%d %s",len, *p));
        return( MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
    }
    if(memcmp(*p, "1dHRsc2NjbHltbGx3eWh5", fix_len) != 0) {
        MBEDTLS_SSL_DEBUG_MSG( 1, ("server psk hint err:%s", *p));
        return( MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
    }
    memcpy(ssl->hint_randomX, *p + fix_len, 16);
    if(ssl->psk_cb != NULL)
        ssl->psk_cb(ssl);
...
// do nth
#else
           mbedtls_ssl_config *p_conf = (mbedtls_ssl_config *)(ssl->conf);
           mbedtls_x509_crt_free( p_conf->ca_chain );
           p_conf->ca_chain = NULL;
           mbedtls_ssl_free_session_negotiate_peer_cert(ssl);
#endif
